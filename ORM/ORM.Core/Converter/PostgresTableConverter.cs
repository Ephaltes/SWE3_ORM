...
{
    public class PostgresTableConverter : ITableConverter
    {
    ...
    protected T DataRowToObject<T>(DataRow row) where T : class, new()
    {
        ...
        foreach (DataColumn dc in row.Table.Columns)
        {
            ...
            {
                ColumnModel col = table.Columns.Where(c =>
                    c.ColumnName.Equals(dc.ColumnName, StringComparison.InvariantCultureIgnoreCase)).First();
                object val = row[dc.ColumnName];
                string? propName = table.GetPropertyNameFromColumnName(dc.ColumnName);

                if (string.IsNullOrEmpty(propName))
                    throw new ArgumentException("Unable to find property in type '" + typeof(T).Name +
                                                "' for column '" + dc.ColumnName + "'.");
